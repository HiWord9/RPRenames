...
public class RenameButton extends Screen {
    ...
    public void drawElements(DrawContext context, int mouseX, int mouseY) {
        if (active) {
            icon.paint(context, -128, 32 + ((buttonHeight + 2) * orderOnPage), 16);
            WLabel shadow = new WLabel(displayText.getText(), 0x3f3f3f);
            shadow.setHorizontalAlignment(HorizontalAlignment.CENTER).paint(context, -70, 38 + (buttonHeight + buttonOffsetY) * orderOnPage, mouseX, mouseY);
            displayText.setHorizontalAlignment(HorizontalAlignment.CENTER).paint(context, -71, 37 + (buttonHeight + buttonOffsetY) * orderOnPage, mouseX, mouseY);
        }
    }

    public void highlightSlot(DrawContext context, ArrayList<Integer> currentInvOrder, ArrayList<String> inventory, String currentItem, int highlightColor) {
        ...
        if (orderInList + 1 <= currentInvOrder.size()) {
            ...
            context.fill(x, y, x + slotSize, y + slotSize, highlightColor);
        }
    }

    public void drawPreview(DrawContext context, int x, int y, int width, int height, double scaleFactorItem, double scaleFactorEntity) {
        int xOffset = 8;
        int yOffset = 2;
        ...
        if (!CEM) {
            if (InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT) != config.playerPreviewByDefault) {
                playerPreview(context, x + xOffset, y + yOffset, (int) (width * scaleFactorEntity), (int) (height * scaleFactorEntity), (int) (32 * scaleFactorEntity), config.spinPlayerPreview, item);
            } else {
                itemPreview(context, x + xOffset, y + yOffset, (int) (width / 2 * scaleFactorItem), (int) (height / 2 * scaleFactorItem), (int) (16 * scaleFactorItem));
            }
        } else {
            entityPreview(context, x + xOffset, y + yOffset, (int) (width * scaleFactorEntity), (int) (height * scaleFactorEntity), (int) (32 * scaleFactorEntity), config.spinMobPreview);
        }
    }
    ...
    private void itemPreview(DrawContext context, int x, int y, int width, int height, int size) {
        drawFakeTooltip(context, x, y, width, height);
        icon.paint(context, x + width / 2 - size / 2, y + height / 2 - size / 2, size);
    }

    private void entityPreview(DrawContext context, int x, int y, int width, int height, int size, boolean spin) {
        ...
        drawFakeTooltip(context, x, y, newWidth, newHeight);
        renderEntity(context, x + newWidth / 2, (int) (y + newHeight - size * 0.75), size, entity, spin);
    }

    private void renderEntity(DrawContext context, int x, int y, int size, Entity entity, boolean spin) {
        ...
        context.getMatrices().translate(x, y, 1050);
    ...
    private void playerPreview(DrawContext context, int x, int y, int width, int height, int size, boolean spin, ItemStack item) {
        ...
        int newHeight = (int) (height + size * entity.getHeight() - 1);
        drawFakeTooltip(context, x, y, newWidth, newHeight);
        renderPlayer(context, x + newWidth / 2, (int) (y + newHeight - size * 0.75), size, entity, spin);
    ...
    private void renderPlayer(DrawContext context, int x, int y, int size, LivingEntity entity, boolean spin) {
        DiffuseLighting.disableGuiDepthLighting();
        context.getMatrices().push();
        context.getMatrices().translate(x, y, 1050);
        context.getMatrices().scale(1f, 1f, -1);
    ...
    public void drawFakeTooltip(DrawContext context, int x, int y, int width, int height) {
        int alpha = 239;
        ...
        int x2 = x + width - 1;
        int y2 = y + height - 1;
        context.fill(x + 1, y, x2 - 1 + 1, y + 1, bgColor);
        context.fill(x + 1, y2, x2 - 1 + 1, y2 + 1, bgColor);
        context.fill(x, y + 1, x + 1, y2 - 1 + 1, bgColor);
        context.fill(x2, y + 1, x2 + 1, y2 - 1 + 1, bgColor);
        context.fill(x + 2, y + 2, x2 - 2 + 1, y2 - 2 + 1, bgColor);
        context.fill(x + 2, y + 1, x2 - 2 + 1, y + 1 + 1, topColor);
        context.fill(x + 2, y2 - 1, x2 - 2 + 1, y2 - 1 + 1, bottomColor);
        context.fillGradient(x + 1, y + 1, x + 1 + 1, y2 - 1 + 1, topColor, bottomColor);
        context.fillGradient(x2 - 1, y + 1, x2 - 1 + 1, y2 - 1 + 1, topColor, bottomColor);
    }
